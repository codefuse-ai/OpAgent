...
class ScriptBrowserEnv(Env[dict[str, Observation], Action]):
    ...
    def setup(self, config_file: Path | None = None) -> None:
        ...
        print("self.context_manager ready")
        self.playwright = self.context_manager.__enter__()
        print("chromium.launch start")
        # TODO: 如需使用代理，请配置环境变量 BROWSER_PROXY
        proxy_config = None
...
if __name__ == "__main__":
    script_env = ScriptBrowserEnv()
    # TODO: 配置您的测试配置文件路径
    config_file = os.getenv("TEST_CONFIG_FILE", "./config_files/test_webarena/1.json")
    script_env.reset(options={'config_file': config_file})
