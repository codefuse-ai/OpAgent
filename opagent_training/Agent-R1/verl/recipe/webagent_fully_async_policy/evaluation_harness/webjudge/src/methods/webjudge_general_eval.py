...
async def WebJudge_general_eval(task, input_image_paths, action_thoughts, last_actions, images_path, model,
                                score_threshold):
...
{last_actions}"""
    ...
    if action_thoughts != None:
        text = prompt.format(task=task, last_actions="\n".join(
            f"{i + 1}. {action}. Reasoning: {action_thought}" for i, (action, action_thought) in
            enumerate(zip(last_actions, action_thoughts))), key_points=key_points,
