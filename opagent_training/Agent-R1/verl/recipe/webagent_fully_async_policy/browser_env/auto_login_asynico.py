...
DEFAULT_TIMEOUT = 10 * 60 * 1000 # 60 seconds
...
def _detect_chromium_path():
    """Auto-detect Playwright's installed chromium executable."""
    ...
    chromium_dirs = sorted(
        [d for d in os.listdir(browsers_path) if d.startswith("chromium-") and os.path.isdir(os.path.join(browsers_path, d))],
        reverse=True,
    )
    for d in chromium_dirs:
        for candidate in [
            os.path.join(browsers_path, d, "chrome-linux", "chrome"),
            os.path.join(browsers_path, d, "chrome-linux", "chrome-wrapper"),
            os.path.join(browsers_path, d, "chrome"),
            os.path.join(browsers_path, d, "chrome-wrapper"),
        ]:
            if os.path.isfile(candidate):
                return candidate
    return None

EXECUTABLE_PATH = os.environ.get("PLAYWRIGHT_CHROMIUM_PATH") or _detect_chromium_path()
...
async def async_renew_comb(comb: List[str], auth_folder: str = "./.auth", owner_actor=None) -> None:
    """
    ...
    for attempt in range(max_retries):
        try:
            logger.info(f"Attempt {attempt + 1}/{max_retries} for combination: {comb}")
            ...
            print(f"❌ Attempt {attempt + 1} failed for {comb} with a timeout: {error_message}")
        except Exception as e:
            print(f"❌ Attempt {attempt + 1} failed for {comb} with an unexpected error: {e}")
        ...
        if attempt < max_retries - 1:
            ...
            # 所有重试均失败
            print(f"❌ All {max_retries} attempts failed for {comb}. Giving up.")
...
@with_timeout_legacy(20 * 60) # 10 minutesss
async def renew_comb_worker(comb: List[str], auth_folder: str, owner_actor=None):
    """A worker that acquires the semaphore before running the renewal task."""
    await async_renew_comb(comb, auth_folder, owner_actor)
...
async def async_verif_web(auth_folder: str, url_list = URLS, owner_actor=None) -> None:
    """
    ...
    for i, res in enumerate(results):
        ...
        if isinstance(res, Exception):
            ...
            if res['is_expired']:
                logger.error(f"🔴 [EXPIRED] Cookie for site '{res['meta']['site']}' in file '{os.path.basename(res['meta']['file'])}' is expired.")
            else:
                logger.info(f"✅ [VALID] Cookie for site '{res['meta']['site']}' in file '{os.path.basename(res['meta']['file'])}' is valid.")

@with_timeout_legacy(60 * 60)
async def async_generate_new_cookies(auth_folder: str, owner_actor=None) -> None:
    ...
    for comb in combs_to_generate:
        ...
        task = asyncio.create_task(
            renew_comb_worker(comb, auth_folder, owner_actor=owner_actor)
        )
        tasks.append(task)
    ...
    for i, res in enumerate(results):
        if isinstance(res, Exception):
            logger.error(f"Error in generating cookies for comb {combs_to_generate[i]}: {res}")

    logger.info("\n--- Cookie Generation Complete ---")
    await async_verif_web(auth_folder, owner_actor=owner_actor)
