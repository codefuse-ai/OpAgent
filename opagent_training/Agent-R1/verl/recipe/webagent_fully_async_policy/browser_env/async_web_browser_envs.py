...
import os
from recipe.webagent_fully_async_policy.evaluation_harness.asyns_evaluators import evaluator_router, EvaluatorComb
import urllib.parse
import re
...
from .actions import Action, aexecute_action, get_action_space, aexecute_action_coords
from .processors import VLM_EXP_DEBUG, ObservationMetadata, AsyncImageObservationProcessor
from .trajectory import Trajectory
from .utils import with_timeout_legacy, get_ws_endpoint_list, DetachedPage, StateInfo
from recipe.webagent_fully_async_policy.evaluation_harness.webjudge.src.qwen_vlm_request import describe_status_image_with_shangshu
from recipe.webagent_fully_async_policy.browser_env.utils import get_storage_state_from_config_file
...
def initialize_browser_actor_pool(ws_endpoint_list = None, webbrowser_thread_executor: concurrent.futures.ThreadPoolExecutor = None) -> List[BrowserActor]:
    """初始化并启动 BrowserActor 池。"""
    if ws_endpoint_list is None:
...
class AsyncScriptBrowserEnv(Env[npt.NDArray[np.uint8], Action]):
    ...
    def save_image(self, bbox_img_np, save_name):
        ...
        self.save_img_file = os.path.join(os.environ.get("DEBUG_IMAGE_SAVE_DIR", "/tmp"), f"{save_name}_{timestamp_str}_{self.context_id}.png")
    ...
    async def setup(self, config_file: Optional[Path] = None, *, storage_state=None, start_url=None, options=None) -> None:
        ...
        if storage_state is not None:
            ...
            # 如果依然没有 storage_state，尝试通过 get_storage_state_from_config_file 获取
            if storage_state is None and config_file:
                try:
                    logger.info(f"Attempting to fetch storage state dynamically for config: {config_file}")
                    storage_state = get_storage_state_from_config_file(str(config_file))
                except Exception as e:
                    logger.warning(f"Failed to get storage state from config file {config_file}: {e}")
    ...
    async def areset(
        ...
        if self.config_file:
                ...
                self.config_file, captioning_fn=None
            )
    ...
    def _get_fallback_observation(self, error: str) -> tuple[np.ndarray, str]:
    ...
    async def astep(
        ...
        if self.page.is_closed():
            error = "Browser/Page is not connected or has been closed before step execution."
            print(f"[astep] {error}")
            screenshot, content = self._get_fallback_observation(error)
            page_url_on_fail = "N/A (Browser/Page Closed)"
            return (
            screenshot, 0.0, True, False, 
            {"page": DetachedPage(page_url_on_fail, content), 
            "error": error, 
            "image_description": f"This is a blank image and shows nothing. This might be due to the following errors: {error}"})
        ...
        try:
            ...
            error = f"Step Action failed, Because: {e}"
            print(f"[astep] {error}")
            screenshot, content = self._get_fallback_observation(error)
        ...
        try:
            ...
            error = f"Getting observation failed. Because: {e}"
            print(f"[astep] {error}")
            screenshot, content = self._get_fallback_observation(error)
            is_terminated = "closed" in str(e).lower()
            page_url_on_fail = self.page.url if not self.page.is_closed() else "N/A (Page Closed)"
            return (screenshot, 0.0, is_terminated, False,
                    {"page": DetachedPage(page_url_on_fail, content), "error": error, 
                    "image_description": f"This is a blank image and shows nothing. This might be due to the following errors: {error}"})
        ...
        if "wCap".lower() in EXPERIMENT_NAME.lower() and self.current_step > 4:
            try:
        ...
        return (
            ...
            {"page": detached_page, "error": error, "image_description": image_description},
        )
