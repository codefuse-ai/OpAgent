...
if NVTX_PROFILE:
    try:
        ...
        def sche_mark(sche_output):
            ...
            mark(f"total_tokens={sche_output.total_num_scheduled_tokens},req_id:num_tokens={sche_output.num_scheduled_tokens}")
...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def execute_model(
        ...
        if NVTX_PROFILE:
            th_nvtx_range_push(f"total bs={len(scheduler_output.num_scheduled_tokens)}, P bs={(np.array(list(scheduler_output.num_scheduled_tokens.values())) > 1).sum()}")
        ...
        if not get_pp_group().is_last_rank:
            ...
            if NVTX_PROFILE:
                th_nvtx_range_push("compute logits")
