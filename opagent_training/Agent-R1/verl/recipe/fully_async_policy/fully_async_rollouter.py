...
class FullyAsyncRollouterBase(FullyAsyncRayPPOTrainer):
    """
    ...
    async def _processor_worker(self):
        ...
        """
        print("[FullyAsyncRollouter][Processor] Worker started")
        loop_count = 0
        while True:
            loop_count += 1
            if loop_count % 100 == 0: # Prevent log flooding, print every 100 loops if idle
                 pass
            
            if self.paused or await self._should_pause_generation():
                print(
                    f"[FullyAsyncRollouter][Processor] Pausing... (paused={self.paused})"
                )
                async with self.lock:
                    self.paused = True
                ...
                print("[FullyAsyncRollouter][Processor] All tasks cleared, waiting for resume...")
                async with self.lock:
                    while self.paused:
                        print(f"[FullyAsyncRollouter][Processor] Waiting for resume... Paused={self.paused}", flush=True)
                        self.idle_start_time = time.time()
                        await self.condition.wait()
                print("[FullyAsyncRollouter][Processor] Resumed!")
                continue
            
            # Debug log
            # print(f"[FullyAsyncRollouter][Processor] Loop {loop_count}, Pending: {self.pending_queue.qsize()}, Active: {len(self.active_tasks)}")
            ...
            if not self.cancel_queue.empty():
                ...
                # print("[FullyAsyncRollouter][Processor] Waiting for pending queue...")
                rollout_sample = await self.pending_queue.get()
                # print(f"[FullyAsyncRollouter][Processor] Got sample {rollout_sample.sample_id if hasattr(rollout_sample, 'sample_id') else rollout_sample}")
    ...
    async def _should_pause_generation(self) -> bool:
        ...
        if queue_size >= self.max_queue_size:
            if not self.paused:
                print(
                    f"[FullyAsyncRollouter][ShouldPause] Pausing generation "
        ...
        if self.staleness_samples >= self.max_required_samples:
            if not self.paused:
                print(
                    "[FullyAsyncRollouter][ShouldPause] Pausing generation "
                    f"due to "
    ...
    async def pause(self):
        """pause rollout"""
        print(f"[FullyAsyncRollouter][Public][Pause] Pausing... Current state: paused={self.paused}", flush=True)
        async with self.lock:
            #print(f"[FullyAsyncRollouter][Public][Pause] Acquiring lock to pause...", flush=True)
            self.paused = True
            # Cancel all rollout tasks
            if self.config.async_training.partial_rollout:
                await self.async_rollout_manager.cancel()
            if self.active_tasks:
                #print(f"[FullyAsyncRollouter][Public][Pause] Cancelling {len(self.active_tasks)} active tasks", flush=True)
                await asyncio.gather(*self.active_tasks, return_exceptions=True)
                self.active_tasks.clear()
                #print("[FullyAsyncRollouter][Public][Pause] All active tasks completed", flush=True)
            #print(f"[FullyAsyncRollouter][Public][Pause] Resetting prefix cache...", flush=True)
            await self.async_rollout_manager.reset_prefix_cache()
            self.monitor_loop_trigger = False
            #print(f"[FullyAsyncRollouter][Public][Pause] Lock released", flush=True)

    async def resume(self, dependency_ref: ObjectRef = None):
        print(f"[FullyAsyncRollouter][Public][Resume] Resuming... Dependency: {dependency_ref}", flush=True)
        if dependency_ref is not None:
            try:
                ray.get(dependency_ref)
                print(f"[FullyAsyncRollouter][Public][Resume] Dependency {dependency_ref} ready.", flush=True)
            except Exception as e:
                print(f"[FullyAsyncRollouter][Public][Resume] Dependency {dependency_ref} FAILED: {e}", flush=True)
        
        print("[FullyAsyncRollouter][Public][Resume] Acquiring lock to resume...", flush=True)
        async with self.lock:
            if self.config.async_training.partial_rollout:
                await self.async_rollout_manager.resume()
            self.paused = False
            # Force reset staleness just in case
            self.staleness_samples = (
                len(self.active_tasks)
                + self.result_queue.qsize()
                + self.cancel_queue.qsize()
            ...
            print(f"[FullyAsyncRollouter][Public][Resume] Resumed. Paused={self.paused}, Staleness reset to {self.staleness_samples}", flush=True)
            self.monitor_loop_trigger = True
            self.condition.notify_all()
    ...
    async def get_statistics(self) -> dict:
        ...
        stats = {
            ...
            "monitor/paused": self.paused,  # Add paused status
...
class FullyAsyncRollouter(FullyAsyncRollouterBase):
    pass
