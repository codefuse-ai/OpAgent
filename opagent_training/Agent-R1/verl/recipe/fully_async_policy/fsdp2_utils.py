...
def fsdp2_sharded_load_from_cpu(
    ...
    if current_device_mesh is None and target_spec is None:
        # Both models have no DTensors (e.g. simple LoRA/DDP case), safe to proceed
        pass
    else:
        assert current_device_mesh is not None, "DTensor parameters not initialized in the model to be loaded"
        assert target_spec is not None, "target_spec is None but model has DTensor parameters"
