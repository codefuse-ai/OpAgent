...
class FullyAsyncTrainerBase(FullyAsyncRayPPOTrainer):
    """
    Base class for FullyAsyncTrainer, separated for inheritance.
    ...
    def set_total_train_steps(self, total_train_steps, initial_step=0):
        self.total_train_steps = total_train_steps
        self.progress_bar = tqdm(total=self.total_train_steps, initial=initial_step, desc="Training Progress")
    ...
    def fit(self):
        ...
        """
        # DEBUG BREAKPOINT: 打断点在加载完预训练权重,初始化完数据,开始训练的阶段
        ...
        while True:
            ...
            with marked_timer("step", timing_raw):
                ...
                with marked_timer("process_batch", timing_raw, color="blue"):
                ...
                with marked_timer("log_rollout", timing_raw):
    ...
    def _trigger_parameter_sync_after_step(self, validate: bool = False, global_steps: int = None):
        ...
        print(f"[FullyAsyncTrainer] Starting parameter sync for version {self.current_param_version}. validate={validate}", flush=True)
        
        with marked_timer("timing_s/wait_last_valid", timing_param_sync):
            print(f"[FullyAsyncTrainer] Waiting for last validation...", flush=True)
            ray.get(self.param_synchronizer.wait_last_valid.remote())
            print(f"[FullyAsyncTrainer] Last validation done.", flush=True)
            
        with marked_timer("timing_s/param_sync", timing_param_sync):
            print(f"[FullyAsyncTrainer] Syncing weights...", flush=True)
            ray.get(
                self.param_synchronizer.sync_weights.remote(
                    self.current_param_version, validate=validate, global_steps=global_steps
                )
            )
            print(f"[FullyAsyncTrainer] Weights synced.", flush=True)
            
        self.logger.log(data=timing_param_sync, step=self.current_param_version)


@ray.remote(num_cpus=10)
class FullyAsyncTrainer(FullyAsyncTrainerBase):
    pass
