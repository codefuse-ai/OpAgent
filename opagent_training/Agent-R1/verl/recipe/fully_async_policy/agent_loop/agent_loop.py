...
class FullyAsyncAgentLoopWorkerBase(AgentLoopWorkerBase):
    def __init__(
...
@ray.remote
class FullyAsyncAgentLoopWorker(FullyAsyncAgentLoopWorkerBase):
    pass
...
class FullyAsyncAgentLoopManager(AgentLoopManager):
    ...
    def wake_up(self):
        """
        Override wake_up to handle both sync and async usage.
        If called from a running loop (e.g. inside an actor), schedule the async task.
        """
        try:
            loop = asyncio.get_running_loop()
            loop.create_task(self._async_wake_up())
        except RuntimeError:
    ...
    def sleep(self):
        """
        Override sleep to handle both sync and async usage.
        If called from a running loop (e.g. inside an actor), schedule the async task.
        """
        try:
            loop = asyncio.get_running_loop()
            loop.create_task(self._async_sleep())
        except RuntimeError:
            asyncio.run(self._async_sleep())
