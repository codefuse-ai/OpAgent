...
class vLLMHttpServerBase:
    ...
    async def generate(
        ...
        if "max_tokens" in sampling_params:
            user_max_tokens = sampling_params.pop("max_tokens")
            if user_max_tokens is not None:
                max_tokens = min(max_tokens, user_max_tokens)

        sampling_params["logprobs"] = 0 if sampling_params.pop("logprobs", False) else None
        sampling_params.setdefault("repetition_penalty", self.config.get("repetition_penalty", 1.0))
        # Ensure 'stop' is a plain list (vLLM asserts isinstance(stop, list))
...
class vLLMReplica(RolloutReplica):
    ...
    async def launch_servers(self):
        ...
        server_name_prefix = getattr(self, 'server_name_prefix', 'vllm_server')
        for node_rank in range(nnodes):
            ...
            name = (
                f"{server_name_prefix}_{self.replica_rank}_{node_rank}"
                if not self.is_reward_model
                else f"{server_name_prefix}_reward_{self.replica_rank}_{node_rank}"
            )
