...
from recipe.webagent_fully_async_policy.browser_env.actions import Action, _id2key
from recipe.webagent_fully_async_policy.browser_env.utils import StateInfo
from recipe.webagent_fully_async_policy.evaluation_harness import image_utils
from tqdm import tqdm
import concurrent.futures
import traceback
from recipe.webagent_fully_async_policy.browser_env.actions import _key2id
from recipe.webagent_fully_async_policy.evaluation_harness.helper_functions import (
    PseudoPage,
    get_query_text,
...
from recipe.webagent_fully_async_policy.browser_env.utils import smart_resize, extract_answer, extract_coords_by_index, extract_tool_call_arguments
import math
...
from recipe.webagent_fully_async_policy.evaluation_harness.webjudge.src import vllm_run
...
class Evaluator(object):
    ...
    async def _enforce_retention_policy(self):
        ...
        subdirs = [d for d in subdirs if "val_" not in d.name]
...
class NumericEvaluator(Evaluator):
    ...
    def __init__(self, 
        ...
        super().__init__(eval_tag, save_path, max_saved_trajectories)

        self.webjudge_evaluator = WebjudgeEvaluator(eval_tag, save_path, max_saved_trajectories)
...
class URLExactEvaluator(Evaluator):
    ...
    async def __call__(
        ...
        logger.info(f"[URLEvaluator] answer: {ref_urls}")
        logger.info(f"[URLEvaluator] pred answer: {pred}")
        score = 0.0
        ...
        for target in targets:
            ...
            if target_url.startswith("func"):
                ...
                if "shopping_get_latest_order_url" in func:
                    target_url = shopping_get_latest_order_url()
...
class BboxJudgeEvaluator(Evaluator):
    ...
    async def __call__(
        ...
        try:
            ...
            ground_truth_bbox = [int(ground_truth_bbox[0]/original_width * 1000)  , 
            int(ground_truth_bbox[1]/original_height * 1000), 
            int(ground_truth_bbox[2]/original_width * 1000), 
            int(ground_truth_bbox[3]/original_height * 1000)] # x1, y1, x2, y2
...
def evaluator_router(
    ...
    for eval_type in eval_types:
        match eval_type:
            ...
            case "webjudge":
                evaluators.append(WebjudgeEvaluator())
            case "bbox_judge":
                evaluators.append(BboxJudgeEvaluator())
            case "action_judge":
                evaluators.append(ActionJudgeEvaluator())
            case _:
                ...
                if "coords" in scaled_action.get("arguments", {}):
                    ...
                    if isinstance(orig_coords, list) and len(orig_coords) == 2:
                        scaled_action["arguments"]["coords"] = [
                            int(orig_coords[0] / original_width * 1000),
                            int(orig_coords[1] / original_height * 1000)
                        ]
...
async def offline_eval():
    offline_file_path = os.environ.get("OFFLINE_EVAL_PATH", "./output/webagent_online/")
    val_file_path = os.environ.get("VAL_EVAL_PATH", "./data/val/")
...
async def test_val_506():
    ...
    config_content = {
      ...
      "start_url": f"http://{os.environ.get('WEBHOSTNAME', 'your-webarena-host').replace('http://', '')}:9999/f/pics",
      "geolocation": None,
      ...
      "eval": {
        ...
        "reference_url": f"http://{os.environ.get('WEBHOSTNAME', 'your-webarena-host').replace('http://', '')}:9999/f/earthporn",
        "program_html": [
          {
    ...
    try:
        ...
        webarena_host = os.environ.get("WEBARENA_HOST", "your-webarena-host")
        evaluator = URLExactEvaluator()
        
        # Load storage state if available to handle login
        storage_state_path = f"./.auth/reddit_state.json"
        context_args = {}
        ...
        async with async_playwright() as p:
            ...
            target_url = f"http://{webarena_host}:9999/f/EarthPorn/2/amazing-shot-of-a-blue-jay-pestering-a-bald-eagle"
...
if __name__ == '__main__':
    ...
    # ]
    # config_file = "./config_files/test_webarena_conflict/1.json"
    # page = None
