...
from recipe.webagent_fully_async_policy.browser_env.utils import (
    AccessibilityTree,
...
class AsyncImageObservationProcessor(ImageObservationProcessor):
    ...
    async def async_get_page_bboxes(self, apage: APage) -> tuple[str, bool]:
        """
        获取页面元素的边界框信息
        
        Returns:
            tuple[str, bool]: (bbox_json_str, page_navigated)
                - bbox_json_str: bbox 数据的 JSON 字符串
                - page_navigated: 是否检测到页面导航并恢复
        """
        js_script = """
        (() => {
            // 1. 更简单、更广泛的选择器
        ...
        # 添加重试机制来处理页面导航导致的执行上下文销毁
        BBOX_RETRIES = 5
        BBOX_RETRY_DELAY = 2
        last_exception = None
        page_navigated = False
        
        for attempt in range(BBOX_RETRIES):
            try:
                # 检查页面是否还有效
                if apage.is_closed():
                    logger.error(f"context_id {self.context_id} [async_get_page_bboxes] Page 已关闭，无法继续")
                    raise RuntimeError("Page was closed during bbox retrieval")
                
                # 如果检测到导航（从第二次尝试开始），等待页面加载完成
                if attempt > 0:
                    try:
                        logger.info(f"context_id {self.context_id} url {apage.url} 等待页面加载完成...")
                        await apage.wait_for_load_state('domcontentloaded', timeout=10000)
                        page_navigated = True  # 标记发生了页面导航
                        logger.info(f"context_id {self.context_id} url {apage.url} 页面加载完成")
                    except Exception as load_error:
                        logger.warning(f"context_id {self.context_id} 等待页面加载时出错: {load_error}")
                
                logger.debug(f"context_id {self.context_id} url {apage.url} [async_get_page_bboxes] 尝试 {attempt + 1}/{BBOX_RETRIES}")
                bbox_result = await apage.evaluate(js_script)
                logger.debug(f"context_id {self.context_id} url {apage.url} [async_get_page_bboxes] 成功获取 bbox 数据")
                return bbox_result, page_navigated
                ...
                # 检查是否是执行上下文被销毁的错误
                if "Execution context was destroyed" in error_message or \
                   "Target closed" in error_message or \
                   "navigation" in error_message.lower():
                    logger.warning(
                        f"context_id {self.context_id} url {apage.url} "
                        f"[async_get_page_bboxes] 尝试 {attempt + 1}/{BBOX_RETRIES} 失败: "
                        f"检测到页面导航或上下文销毁"
                    )
                    if attempt < BBOX_RETRIES - 1:
                        logger.info(f"[async_get_page_bboxes] 将在 {BBOX_RETRY_DELAY} 秒后重试...")
                        await asyncio.sleep(BBOX_RETRY_DELAY)
                        continue
                else:
                    # 其他类型的错误，不重试
                    logger.error(f"context_id {self.context_id} url {apage.url} [async_get_page_bboxes] 发生未预料的错误: {e}")
                    break
        
        # 所有重试都失败，返回空的 bbox 数据
        logger.error(
            f"context_id {self.context_id} url {apage.url} "
            f"[async_get_page_bboxes] 所有重试失败: {last_exception}, 使用空的 bbox 数据"
        )
    ...
    async def async_fetch_browser_info(self, apage: APage) -> Dict[str, Any]:
        ...
        # --- 重试配置 ---
        max_retries = 5      # 增加到5次重试，因为页面导航可能需要更多时间
        retry_delay_seconds = 2  # 增加到2秒，给页面更多加载时间
            ...
            try:
                ...
                if attempt > 0:
                    try:
                        logger.info(f"context_id {self.context_id} url {apage.url} 等待页面加载完成...")
                        await apage.wait_for_load_state('domcontentloaded', timeout=10000)
                        logger.info(f"context_id {self.context_id} url {apage.url} 页面加载完成，继续尝试获取 DOM Tree")
                    except Exception as load_error:
                        logger.warning(f"context_id {self.context_id} 等待页面加载时出错: {load_error}")
                ...
                if "Execution context was destroyed" in error_message or \
                   "Target closed" in error_message:
                    ...
                    logger.warning(
                        f"context_id {self.context_id} url {apage.url} "
                        f"尝试 {attempt + 1}/{max_retries}: "
                        f"在获取信息时检测到页面跳转/上下文销毁。将在 {retry_delay_seconds} 秒后重试..."
                    )
                    if attempt < max_retries - 1:
                        await asyncio.sleep(retry_delay_seconds)
                        # 循环将继续下一次尝试，并等待页面加载完成
                        continue
                    else:
                        # 这是最后一次尝试
                        logger.error(
                            f"context_id {self.context_id} url {apage.url} "
                            f"已达到最大重试次数({max_retries})。由于反复的页面跳转，获取浏览器信息失败。"
                        )
                        default_error_return["config"]["error"] = "已达到最大重试次数。执行上下文被反复销毁。"
                        break # 退出循环，以返回默认错误
                else:
    ...
    def save_image(self, bbox_img, save_name):
        ...
        self.save_img_file = os.path.join(os.environ.get("DEBUG_IMAGE_SAVE_DIR", "/tmp"), f"{save_name}_{timestamp_str}.png")
    ...
    async def async_process(self, apage: APage) -> tuple[np.ndarray, str]:
        ...
        try:
            ...
            screenshot_img = Image.open(BytesIO(screenshot_bytes)).convert("RGB")
            if self.observation_type == "image_som":
                # 调用内置重试机制的 async_get_page_bboxes
                som_bboxes, page_navigated = await self.async_get_page_bboxes(apage)
                
                # 如果检测到页面导航，重新截图以确保 screenshot 和 bbox 对应同一个页面状态
                if page_navigated:
                    try:
                        logger.info(f"context_id {self.context_id} url {apage.url} 检测到页面导航，重新获取screenshot")
                        screenshot_bytes = await apage.screenshot(timeout=DEFAULT_TIMEOUT)
                        screenshot_img = Image.open(BytesIO(screenshot_bytes)).convert("RGB")
                    except Exception as screenshot_error:
                        logger.warning(f"context_id {self.context_id} 重新截图失败: {screenshot_error}")
