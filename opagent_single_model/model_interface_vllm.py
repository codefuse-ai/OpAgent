#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""vLLM-backed model interface for OpAgent.

Loads the full-precision codefuse-ai/OpAgent (Qwen3-32B) model locally via
vLLM and runs inference in-process.  This is the **full-precision** path:

    Browser screenshot  →  vLLM in-process inference  →  parsed action

Usage requirements:
    - A machine with sufficient GPU VRAM (e.g. 2× A100 80 GB for 32B)
    - ``pip install vllm transformers qwen_vl_utils``

Configuration (edit MODEL_CONFIG dict or call set_model_config() at runtime):
    model_path              codefuse-ai/OpAgent
...
def smart_resize(height: int, width: int, factor: int = 28, min_pixels: int = 784, max_pixels: int = 3211264) -> Tuple[int, int]:
    """Calculate compressed image size for model input (consistent with qwen_vl_utils)."""
...
def process_image(image: Union[bytes, str, ImageObject], max_pixels: int, min_pixels: int) -> Tuple[ImageObject, float, float, Tuple[int, int], Tuple[int, int]]:
    """Process image and calculate scaling ratio.
...
def load_model():
    ...
    GPU Configuration Notes:
        - tensor_parallel_size: Load model across multiple GPUs in parallel.
        - gpu_memory_utilization: GPU memory usage ratio per GPU.
        - Set CUDA_VISIBLE_DEVICES to select GPUs, e.g. export CUDA_VISIBLE_DEVICES=0,1,2,3
    """
...
def build_system_prompt() -> str:
    """Build system prompt."""
    system_prompt = (
        "You are an excellent web agent. Based on the web screen shot and content, "
        "your need call the single, most appropriate tool for the current step to make progress on the user's request."
    )
    system_prompt += (
        "\nOutput the thinking process in <think> </think> tags, and for each function call, "
...
def convert_to_model_format(action_type: str, params: Dict, original_width: int = 1280, original_height: int = 800) -> Dict:
    ...
    if x is not None and y is not None:
        result["arguments"]["coords"] = [int(x / original_width * 1000), int(y / original_height * 1000)]
...
def convert_from_model_format(model_output: Dict, original_width: int = 1280, original_height: int = 800) -> Dict:
    """Convert model output format to action_executor format.
...
def parse_model_response(raw_response: str, original_width: int = 1280, original_height: int = 800) -> Tuple[Optional[str], Optional[Dict]]:
    ...
    Returns:
        (think_content, tool_call) - tool_call is in action_executor format
    """
    think_content = None
    tool_call = None
...
def call_model(model_input: ModelInput) -> ModelOutput:
    ...
    Returns:
        ModelOutput with think content and tool_call.
    """
    logger.info(f"[vLLM] Calling OpAgent with query: {model_input.query[:50]}...")
    logger.info(f"[vLLM] History length: {len(model_input.history)}")
    logger.info(f"[vLLM] Current URL: {model_input.current_url}")
    ...
    try:
        ...
        logger.info(f"[vLLM] Screenshot {original_width}x{original_height} → model input {resized_size[0]}x{resized_size[1]}")
        ...
        logger.info(f"[vLLM] Response length: {len(raw_response)}")
        logger.debug(f"[vLLM] Raw response: {raw_response}")

    except Exception as e:
        logger.error(f"[vLLM] Model call failed: {e}")
        import traceback
        traceback.print_exc()

        raw_response = (
            f'<think>Model call failed: {e}</think>'
            f'<tool_call>{{"name": "answer", "arguments": {{"content": "Model call failed"}}}}</tool_call>'
        )
        original_width = 1280
        original_height = 800
    ...
    if think_content:
        logger.info(f"[vLLM] Think: {think_content[:100]}...")
    if tool_call:
        logger.info(f"[vLLM] Tool call: {tool_call}")
    else:
        logger.warning("[vLLM] No valid tool_call found in response")
...
def set_model_config(model_path: str = None,
    ...
    """Dynamically modify model configuration and reset cached model instances."""
    global _llm, _processor, _sampling_params
    ...
    logger.info(f"[vLLM] Config updated: {MODEL_CONFIG}")
